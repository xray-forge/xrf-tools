...
fn a_texture_without_a_descriptor_is_undeclared_and_flat() {
  let descriptor: XrayMaterialDescriptor = describe(&FixtureTree::new("no_descriptor").with_texture(BASE));
...
fn a_bump_map_typed_descriptor_is_skipped_whole_however_complete_its_declaration() {
  ...
  let descriptor: XrayMaterialDescriptor = describe(
    &FixtureTree::new("type_disqualified")
      .with_texture(BASE)
      .with_texture(BUMP)
...
fn a_rejected_reference_is_undeclared() {
  let descriptor: XrayMaterialDescriptor = {
    let tree: FixtureTree = FixtureTree::new("rejected").with_texture(BASE);
...
fn describing_a_located_descriptor_answers_the_same_as_describing_its_texture() {
  let tree: FixtureTree = FixtureTree::new("two_doors")
    .with_texture(BASE)
    .with_texture(BUMP)
    .with_texture(COMPANION)
...
fn a_textures_ltx_in_the_roots_is_named_and_otherwise_unread() {
  ...
  let tree: FixtureTree = FixtureTree::new("textures_ltx").with_texture(BASE);
  ...
  let bare: FixtureTree = FixtureTree::new("no_textures_ltx").with_texture(BASE);
