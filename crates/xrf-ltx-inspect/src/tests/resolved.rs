...
use crate::resolved::{LtxResolvedFieldOrigin, LtxResolvedIndex, LtxResolvedReader, LtxResolvedSection};
...
fn a_field_two_levels_up_reports_the_section_that_writes_it() -> XrfResult {
  ...
  let sections: Vec<LtxResolvedSection> =
    LtxResolvedReader::new("system.ltx", "ltx", &resolution, &source).read_sections(&["wpn_child"])?;
...
fn the_index_carries_declared_parents_that_resolving_flattened_away() -> XrfResult {
  ...
  let resolution: LtxResolution = source.resolve("system.ltx")?;
  let index: LtxResolvedIndex = LtxResolvedReader::new("system.ltx", "ltx", &resolution, &source).read_index()?;
...
fn a_section_the_root_does_not_hold_is_skipped_rather_than_refused() -> XrfResult {
  ...
  let resolution: LtxResolution = source.resolve("system.ltx")?;
  let sections: Vec<LtxResolvedSection> =
    LtxResolvedReader::new("system.ltx", "ltx", &resolution, &source).read_sections(&["wpn_base", "wpn_absent"])?;
