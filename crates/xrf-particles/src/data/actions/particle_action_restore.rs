...
mod tests {
  ...
  fn test_read_write() -> XrfResult {
    ...
    let original: ParticleActionRestore = ParticleActionRestore {
      action_flags: 1,
    ...
    assert_eq!(ParticleActionRestore::read::<XRayByteOrder, _>(&mut reader)?, original);
  ...
  fn test_import_export() -> XrfResult {
    ...
    assert_eq!(ParticleActionRestore::import("data", &source)?, original);
  ...
  fn test_serialize_deserialize() -> XrfResult {
    ...
    assert_eq!(original, serde_json::from_str::<ParticleActionRestore>(&serialized)?);
