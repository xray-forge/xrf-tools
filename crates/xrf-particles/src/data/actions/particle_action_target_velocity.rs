...
mod tests {
  ...
  fn test_read_write() -> XrfResult {
    ...
    let original: ParticleActionTargetVelocity = ParticleActionTargetVelocity {
      action_flags: 1,
      action_type: ParticleActionType::TargetVelocity,
    ...
    assert_eq!(file.bytes_remaining(), 24 + 8);

    let mut reader: ChunkReader = ChunkReader::from_slice(file)?.read_child_by_index(0)?;

    assert_eq!(
      ParticleActionTargetVelocity::read::<XRayByteOrder, _>(&mut reader)?,
      original
    );
  ...
  fn test_import_export() -> XrfResult {
    ...
    assert_eq!(ParticleActionTargetVelocity::import("data", &source)?, original);
  ...
  fn test_serialize_deserialize() -> XrfResult {
    ...
    assert_eq!(
      original,
      serde_json::from_str::<ParticleActionTargetVelocity>(&serialized)?
    );
