...
impl ChunkReadWrite for ParticleActionMatchVelocity {
...
impl LtxImportExport for ParticleActionMatchVelocity {
...
mod tests {
  ...
  fn test_read_write() -> XrfResult {
    ...
    let original: ParticleActionMatchVelocity = ParticleActionMatchVelocity {
      action_flags: 1,
      action_type: ParticleActionType::MatchVelocity,
      magnitude: 1.52,
      epsilon: 0.00003,
      max_radius: 41.5,
    };
    ...
    assert_eq!(
      ParticleActionMatchVelocity::read::<XRayByteOrder, _>(&mut reader)?,
      original
    );
  ...
  fn test_import_export() -> XrfResult {
    ...
    assert_eq!(ParticleActionMatchVelocity::import("data", &source)?, original);
  ...
  fn test_serialize_deserialize() -> XrfResult {
    ...
    assert_eq!(
      original,
      serde_json::from_str::<ParticleActionMatchVelocity>(&serialized)?
    );
