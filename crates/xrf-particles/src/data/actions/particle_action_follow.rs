...
mod tests {
  ...
  fn test_read_write() -> XrfResult {
    ...
    let original: ParticleActionFollow = ParticleActionFollow {
      action_flags: 1,
      action_type: ParticleActionType::Follow,
      magnitude: 240.45,
      epsilon: 0.00001,
      max_radius: 4.05,
    };
    ...
    assert_eq!(ParticleActionFollow::read::<XRayByteOrder, _>(&mut reader)?, original);
  ...
  fn test_serialize_deserialize() -> XrfResult {
    ...
    assert_eq!(original, serde_json::from_str::<ParticleActionFollow>(&serialized)?);
