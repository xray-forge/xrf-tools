...
mod tests {
  ...
  fn test_read_write() -> XrfResult {
    ...
    let original: ParticleActionRandomDisplace = ParticleActionRandomDisplace {
      action_flags: 1,
      action_type: ParticleActionType::RandomDisplace,
      gen_disp: ParticleDomain::new_mock(),
    };
    ...
    assert_eq!(
      ParticleActionRandomDisplace::read::<XRayByteOrder, _>(&mut reader)?,
      original
    );
  ...
  fn test_import_export() -> XrfResult {
    ...
    assert_eq!(ParticleActionRandomDisplace::import("data", &source)?, original);
  ...
  fn test_serialize_deserialize() -> XrfResult {
    ...
    assert_eq!(
      original,
      serde_json::from_str::<ParticleActionRandomDisplace>(&serialized)?
    );
