...
mod tests {
  ...
  fn test_read_write() -> XrfResult {
    ...
    let original: ParticleActionMove = ParticleActionMove {
      action_flags: 1,
    ...
    assert_eq!(ParticleActionMove::read::<XRayByteOrder, _>(&mut reader)?, original);
  ...
  fn test_import_export() -> XrfResult {
    ...
    assert_eq!(ParticleActionMove::import("data", &source)?, original);
  ...
  fn test_serialize_deserialize() -> XrfResult {
    ...
    assert_eq!(original, serde_json::from_str::<ParticleActionMove>(&serialized)?);
