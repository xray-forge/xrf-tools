...
mod tests {
  ...
  fn test_read_write() -> XrfResult {
    ...
    let original: ParticleActionOrbitLine = ParticleActionOrbitLine {
      action_flags: 1,
      action_type: ParticleActionType::OrbitLine,
    ...
    assert_eq!(
      ParticleActionOrbitLine::read::<XRayByteOrder, _>(&mut reader)?,
      original
    );
  ...
  fn test_import_export() -> XrfResult {
    ...
    assert_eq!(ParticleActionOrbitLine::import("data", &source)?, original);
  ...
  fn test_serialize_deserialize() -> XrfResult {
    ...
    assert_eq!(original, serde_json::from_str::<ParticleActionOrbitLine>(&serialized)?);
