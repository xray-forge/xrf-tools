...
use crate::spawn::level_spawn_object::LevelSpawnObject;

/// The objects a level spawns, `level.spawn`.
///
/// Not a spawn set: `xrServer::SLS_Default` walks the file as a flat run of chunks and hands each one straight to
/// `Process_spawn` as a network packet, so there is no header, no graph and no identity - just the objects. A set is
/// what `xrAI` builds out of every level's list, and it is the other format that shares this extension.
