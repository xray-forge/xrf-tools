...
//!
//! A volume is a chunked file like any other, so the header itself - its widths and the compression mark - belongs to
//! `xrf-chunk` and is re-exported from this crate's root beside the ids below, which are the volume's own.
