...
use crate::core::session::{DocumentRestore, DocumentSession, DocumentSessionId, DocumentSnapshot};

#[test]
fn a_failed_replacement_keeps_the_committed_document_readable() {
  let session: DocumentSession<String> = DocumentSession::new("test");
...
#[test]
fn an_old_teardown_cannot_close_a_new_owners_document_or_opening() {
  let session: DocumentSession<&str> = DocumentSession::new("test");
  let old: DocumentSessionId = DocumentSessionId::new();
  let new: DocumentSessionId = DocumentSessionId::new();
...
fn an_accepted_snapshot_outlives_close_and_a_stale_save_cannot_publish() {
  ...
  let id: DocumentSessionId = DocumentSessionId::new();
  session.begin_open(id).unwrap();
  let held: Arc<DocumentSnapshot<String>> = session.commit_open(id, String::from("before")).unwrap();
...
#[test]
fn restoration_and_open_serialize_the_same_identity_and_document() {
  let session: DocumentSession<Vec<u8>> = DocumentSession::new("wire test");
  let id: DocumentSessionId = DocumentSessionId::new();
  session.begin_open(id).unwrap();
  let opened = session.commit_open(id, vec![1, 2, 3]).unwrap();
  let restored = DocumentRestore::from(session.get().unwrap());
  let expected = serde_json::json!({ "sessionId": id, "document": [1, 2, 3] });
  assert_eq!(serde_json::to_value(&opened).unwrap(), expected);
  assert_eq!(serde_json::to_value(&restored).unwrap(), expected);
  session.close(&[id]).unwrap();
  assert_eq!(
    serde_json::to_value(DocumentRestore::from(session.get().unwrap())).unwrap(),
...
fn reloading_checks_the_previous_identity_before_reserving_publication() {
  let session: DocumentSession<&str> = DocumentSession::new("reload test");
  let first: DocumentSessionId = DocumentSessionId::new();
  let second: DocumentSessionId = DocumentSessionId::new();
  session.begin_open(first).unwrap();
  ...
  assert!(
    session
      .begin_reload(DocumentSessionId::new(), DocumentSessionId::new())
      .is_err()
  );
