...
pub async fn dialogs_detect_mode(world: XrayWorldSpec) -> TauriResult<DialogProjectMode> {
  let mode: DialogProjectMode = detect_mode(&world).map_err(error_to_string)?;
