...
fn pending_replacement_keeps_the_committed_file_readable() {
  ...
  let first = state.begin_open().unwrap();
  let descriptor = state
    .commit_open(first, "first.spawn".into(), synthetic_spawn())
    .unwrap();
  let snapshot = state.require(first).unwrap();
  let next = state.begin_open().unwrap();

  assert_eq!(state.get_descriptor().unwrap().unwrap().id, first);
  ...
  assert_eq!(snapshot.descriptor.id, first);
}

#[test]
fn newer_open_prevents_an_older_open_from_committing() {
  let state = SpawnFileState::new();
  let older = state.begin_open().unwrap();
  let newer = state.begin_open().unwrap();
  ...
  assert!(state.commit_open(older, "old.spawn".into(), synthetic_spawn()).is_err());
  assert_eq!(state.get_descriptor().unwrap().unwrap().id, newer);
}
...
fn close_invalidates_pending_opens_and_reads_but_preserves_acquired_snapshots() {
  ...
  state
    ...
    .unwrap();
  let snapshot = state.require(first).unwrap();
  let pending = state.begin_open().unwrap();
  ...
  let reopened = state.begin_open().unwrap();
  state
...
fn packed_and_unpacked_opens_preserve_the_session_when_a_replacement_fails() {
  ...
  tauri::async_runtime::block_on(async {
    for (path, input) in [(packed, SpawnInput::Packed), (unpacked, SpawnInput::Unpacked)] {
      let opened = open_spawn(path.clone(), input, &state, &execution).await.unwrap();
      assert_eq!(opened.path, path);
      assert_eq!(
        serde_json::to_value(&state.require(opened.id).unwrap().file).unwrap(),
        serde_json::to_value(&source).unwrap()
      );

      for input in [SpawnInput::Packed, SpawnInput::Unpacked] {
        assert!(open_spawn(corrupt.clone(), input, &state, &execution).await.is_err());
        assert_eq!(state.get_descriptor().unwrap().unwrap().id, opened.id);
        assert!(state.require(opened.id).is_ok());
      }
    }
  });
}
