...
/**
 * Installs the repository's git hooks, from the workspace root's `pnpm install`.
 */
...
if (repositoryRoot) {
  ...
  const hooks = path
    .relative(repositoryRoot, path.join(WORKSPACE_ROOT, "cli", ".husky"))
    .split(path.sep)
    .join("/");
  ...
  if (failure) {
    console.error(`xrf-tools: commit hooks are not installed (${failure}).`);
  }
}
